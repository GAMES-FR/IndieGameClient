...
public:
	...
	virtual bool OnEvent(const SEvent& event)
	{
		...
		//state of the first joystick, ignoring othe joysticks.
...
int main()
{
	...
	if (device == 0)
		return 1; //could note create selected driver

	core::array<SJoystickInfo> joystickInfo;
	if (device->activateJoysticks(joystickInfo))
	{
		std::cout << "Joystick support is enabled and " << joystickInfo.size() << "jystick(s) are present." << std::endl;
	...
	while (device->run())
	{
		...
		if (joystickInfo.size() > 0)
		{
			...
			//gauranted to be 65535 if it's not supported, so we can check its range.
			const u16 povDegrees = JoystickData.POV / 100;
			if (povDegrees < 360)
			{
				if (povDegrees > 0 && povDegrees < 100)
					moveHorizontal = 1.f;
		...
		if (!movedWithJoystick)
		{
			...
			core::line3df ray = smgr->getSceneCollisionManager()->getRayFromScreenCoordinates(receiver.GetMouseState().Position, camera);
